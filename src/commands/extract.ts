...
export async function extractStaleFromDB(
  ...
  for (;;) {
    ...
    // A managed brain's timeline is a canonical projection written by page publication
    // (persistence/canonical-projections.ts); only the unmanaged legacy path fills it here.
    if (!managed) for (let i = 0; i < timelineRows.length; i += BATCH_SIZE) {
